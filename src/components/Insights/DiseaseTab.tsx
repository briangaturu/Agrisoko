...
// const normalizeDiseaseResults = (payload: any): DiseaseResult[] => {
//   const source = payload?.data ?? payload?.predictions ?? payload?.results ?? payload;
...
//       if (res.status === 503) throw new Error(json?.error || "Disease model is temporarily unavailable.");
//       if (!res.ok)            throw new Error(json?.error || "Detection failed");

//       const normalized = normalizeDiseaseResults(json);
//       if (normalized.length === 0) {
//         throw new Error("No disease predictions returned. Try a clearer crop image.");
//       }

//       setResults(normalized.slice(0, 5));
//     } catch (err: any) {
